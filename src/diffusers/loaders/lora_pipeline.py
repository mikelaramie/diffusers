...
class HunyuanVideo15LoraLoaderMixin(LoraBaseMixin):
    r"""
    Load LoRA layers into [`HunyuanVideo15Transformer3DModel`]. Specific to [`HunyuanVideo15Pipeline`] and
    [`HunyuanVideo15ImageToVideoPipeline`].

    Expected checkpoints are Diffusers/PEFT format (every key contains `"lora"`). Original HunyuanVideo v1 packs that
    use `img_attn_qkv` are not supported — load those with [`HunyuanVideoLoraLoaderMixin`].
    """
    ...
    def lora_state_dict(
        ...
        **kwargs,
        ...
        if any("img_attn_qkv" in k for k in state_dict):
            raise ValueError(
                "This checkpoint looks like an original HunyuanVideo v1 LoRA (`img_attn_qkv` keys). "
                "Load it with `HunyuanVideoPipeline` / `HunyuanVideoLoraLoaderMixin`, not HunyuanVideo 1.5."
            )
    ...
    @require_peft_backend
    # Copied from diffusers.loaders.lora_pipeline.HunyuanVideoLoraLoaderMixin.load_lora_weights
    def load_lora_weights(
        self,
        pretrained_model_name_or_path_or_dict: str | dict[str, torch.Tensor],
        adapter_name: str | None = None,
    ...
    @classmethod
    # Copied from diffusers.loaders.lora_pipeline.HunyuanVideoLoraLoaderMixin.load_lora_into_transformer
    def load_lora_into_transformer(
        cls,
        state_dict,
        transformer,
    ...
    @classmethod
    # Copied from diffusers.loaders.lora_pipeline.HunyuanVideoLoraLoaderMixin.save_lora_weights
    def save_lora_weights(
        cls,
        save_directory: str | os.PathLike,
        transformer_lora_layers: dict[str, torch.nn.Module | torch.Tensor] = None,
    ...
    # Copied from diffusers.loaders.lora_pipeline.HunyuanVideoLoraLoaderMixin.fuse_lora
    def fuse_lora(
        self,
        components: list[str] = ["transformer"],
        lora_scale: float = 1.0,
    ...
    # Copied from diffusers.loaders.lora_pipeline.HunyuanVideoLoraLoaderMixin.unfuse_lora
    def unfuse_lora(self, components: list[str] = ["transformer"], **kwargs):
        r"""
        See [`~loaders.StableDiffusionLoraLoaderMixin.unfuse_lora`] for more details.
        """
